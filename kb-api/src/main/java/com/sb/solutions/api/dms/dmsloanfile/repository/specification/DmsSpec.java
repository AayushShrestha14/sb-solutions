...
public class DmsSpec implements Specification<DmsLoanFile> {
    ...
    @Override
    public Predicate toPredicate(Root<DmsLoanFile> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        Date date = new Date();
        switch (property) {
            case FILTER_BY_DATE:
                try {
            ...
            case FILTER_BY_LOAN:
                return criteriaBuilder.and(criteriaBuilder.equal(root.join("loanConfig").get("id"), Long.valueOf(value)));
            default:
                return null;
        }
    }
