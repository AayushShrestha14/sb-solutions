...
public interface CustomerCadRepository extends
    BaseRepository<CustomerApproveLoanCadDocumentation, Long> {

    @Modifying
    @Transactional
    @Query("UPDATE CustomerApproveLoanCadDocumentation  c set c.cadStageList= :previousList,c.docStatus=:docStatus , c.cadCurrentStage = :cadStage WHERE c.id = :id")
