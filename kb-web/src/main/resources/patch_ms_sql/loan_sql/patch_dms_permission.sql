BEGIN DECLARE @countPermission smallint
SET @countPermission = (Select count(*)
                        from permission
                        where id = 2)
DELETE
FROM permission_api_list
where permission_id in (4, 8, 11)
DELETE
FROM role_permission_rights
WHERE permission_id in (4, 8, 11)
DELETE
FROM permission
WHERE id in (4, 8, 11)
...
BEGIN DECLARE @countLoan smallint
DECLARE @countPending smallint
DECLARE @checkPermPending smallint
DECLARE @checkPermLoan smallint
SET @countLoan = (Select count(*)
                  from url_api
                  where id = 40)
SET @checkPermLoan = (Select count(*)
                      from permission_api_list
                      where api_list_id = 40)
SET @checkPermPending = (Select count(*)
                         from permission_api_list
                         where api_list_id = 46)

SET @countPending = (Select count(*)
                     from url_api
                     where id = 46)
    if
    (@countLoan = 0)
