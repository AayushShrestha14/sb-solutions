...
SET @count = (Select count(*)
              from permission
              where id = 4)
SET @chckLoanConfig = (Select count(*)
                       from permission
                       where id = 2)
    if
    (@chckLoanConfig = 0)
...
-- ************************DASHBOARD******************************************
BEGIN DECLARE @chkLoanurlapi smallint
DECLARE @chkNotificationUrlapi smallint
DECLARE @chkPendingurlapi smallint
set @chkLoanurlapi = (select count(*)
                      from url_api
                      where id = 40)
set @chkNotificationUrlapi = (select count(*)
                              from url_api
                              where id = 45)
set @chkPendingurlapi = (select count(*)
                         from url_api
                         where id = 46)
    if
    (@chkLoanurlapi = 0)
...
BEGIN DECLARE @ChkAPiMap smallint
